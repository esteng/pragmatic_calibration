...
def chatify(prompt):
    ...
    prompt = f"""<|start_header_id|>system<|end_header_id|> {prompt.strip()}
<|eot_id|><|start_header_id|>user<|end_header_id|>
Question: {question.strip()}
Provide your response: <|eot_id|><|start_header_id|>assistant<|end_header_id|>"""
    return prompt 

def dechatify(output):
    
    output = re.sub(f"^.*Provide your response: {re.escape('<|eot_id|><|start_header_id|>assistant<|end_header_id|>')}", "Response:", output)
    return output

def run_prompt(prompt): 
    prompt = chatify(prompt) 
    ...
    return [dechatify(x) for x in output_decoded]
...
if __name__ == "__main__":
    ...
    model = AutoModelForCausalLM.from_pretrained(args.trained_model,  device_map="auto")
    model = model.half()
